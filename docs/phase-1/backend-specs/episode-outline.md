...
除“正文创作 / 续写 / 润色”外，上述 AI 辅助能力仍只要求前端展示按钮或面板入口。
...
- 正文创作、续写和润色使用同一持久化候选版本和幂等请求 ID；采用前不得修改正式正文。
- 候选版本记录 `generation_type: create | continue | polish`；旧候选默认视为 `create`。
- 续写和润色要求当前正文已保存且非空，并在同一次文本模型调用中融合 Humanizer-zh 去 AI 味规则。
- 采用候选稿时清空旧摘要和质检备注，将正文状态设为 `draft`，并标记同集制作下游为 `needs_review`。
- 本阶段不新增撤销润色、摘要或质检接口。钩子提取归入短剧制作模块。
