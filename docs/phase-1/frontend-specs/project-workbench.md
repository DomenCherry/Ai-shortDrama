...
- 页面路径：入口页 `/projects/[id]`；模块编辑页 `/projects/[id]/assets`、`/projects/[id]/story-text`、`/projects/[id]/production`。
- 目标：把项目工作台从线性阶段导航调整为三类工作入口：项目资料 / 资产、故事文本、短剧制作。用户点击入口卡片后进入对应模块编辑页，而不是在入口页下方直接编辑。
...
进入项目工作台后，主区域首先展示三个入口。入口页只负责展示项目概览、入口卡片和状态摘要，不直接展开完整编辑表单。

- 项目资料 / 资产：点击进入 `/projects/[id]/assets`。
- 故事文本：点击进入 `/projects/[id]/story-text`。
- 短剧制作：点击进入 `/projects/[id]/production`。

入口应展示简短状态摘要：

- 项目资料 / 资产：是否已加载世界观、已加载角色数量、项目基础信息是否完整。
- 故事文本：整体故事大纲状态、分集大纲完成数量、单集故事正文完成数量。
...
模块编辑页顶部将三者展示为同级导航，不在“世界观与角色”下继续嵌套二级 Tab。世界观入口展示已加载数量和单项目上限（`0/1` 或 `1/1`），角色入口展示当前已加载角色数量。
...
- 按集维护单集故事正文、正文摘要和状态；前文参考只读展示上一集正文摘要。
- 单集故事正文页只读引用分集大纲中的标题、梗概、开场钩子、冲突、反转和结尾悬念；这些写作计划字段仍回到分集大纲中编辑。
- 支持从分集列表选择某一集，在三栏工作台中编辑正文。
- AI 正文创作、续写、润色、撤销润色、摘要、一致性质检、角色参考、设定参考、文风和灵感面板在本阶段只展示前端入口，不调用后端 AI 接口。钩子提取归入短剧制作模块。
- 单集故事正文是短剧制作的主要输入。
- 详细交互见 [分集大纲与单集故事正文创作 Spec](./episode-outline.md)。
...
- 平台简介。
- 传播点 / 发布文案。
- 短剧制作顶部展示【钩子提取】入口，本阶段只展示入口，不调用后端 AI 接口。

短剧制作约束：

- 短剧制作区域不能直接编辑项目世界观和项目角色。
- 短剧制作区域应展示当前沿用的世界观、角色和故事文本摘要，作为只读上下文。
- 如果用户需要修改世界观、角色、整体故事大纲、分集大纲或单集故事正文，应回到项目资料 / 资产或故事文本入口修改。
...
- 摘要至少包括：世界观名称、角色数量、整体故事大纲状态、当前集分集大纲状态、当前集单集故事正文状态。
...
- 保存整体故事大纲、分集大纲、单集故事正文或剧本后，下游对应内容显示"需要检查"。
