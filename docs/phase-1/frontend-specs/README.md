...
前端页面 spec 用于在实现页面前明确页面目标、布局、字段、交互状态、API 依赖、错误提示、桌面端布局要求和验收标准。

第一期前端默认面向桌面端工作台，不要求移动端专门适配；已有窄屏兼容样式可保留，但不作为新增功能或验收标准。
...
- [分集大纲与单集故事正文创作](./episode-outline.md) - 故事文本下的分集大纲、三栏正文创作工作台和 AI 按钮展示
