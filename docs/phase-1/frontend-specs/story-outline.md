...
- 右侧约 2/3 宽度展示对话区域，包括 AI 消息、用户回复输入、字段更新说明和确认保存按钮。

交互规则：

- 页面进入后加载当前正式故事大纲，并调用 AI 协助接口发起首轮引导。
...
- 故事核心层和结构规划层字段完整后，确认保存按钮可用。
- 点击确认保存后调用正式故事大纲保存接口，保存成功后跳回 `/projects/{project_id}/story-text`。
- 点击返回故事大纲时不保存草稿内容。
...
| AI 协助失败 | AI 协助失败 |
| AI 协助回复为空 | 请先输入回复内容 |
| AI 协助必填字段未完成 | 请先完成故事核心层和结构规划层的必填字段 |
